...
const { detectLink, detectVirtex, isSpamMessage } = require('./antispam')
...
fs.watchFile(file, () => {
	...
	require(file)
        ...
        // ======== DETEKSI LINK GRUP (ANTILINK) =========
        if (isGroup && db.chats[m.chat]?.antilink && budy.match(/https:\/\/chat\.whatsapp\.com\//i)) {
            if (!isAdmin && !isBotAdmin) return
            m.reply("Link grup terdeteksi! Pengguna akan dikick.")
        ...
        if (!db.virtex) db.virtex = {};

        const now = Date.now();

        // Antispam - deteksi kirim terlalu cepat
        if (!isOwner && !isPremium) {
            if (!db.antispam[m.sender]) db.antispam[m.sender] = 0
            if ((now - db.antispam[m.sender]) < 3000) {
                m.reply("Kamu terlalu cepat mengirim pesan! Dikenakan tindakan.")
                if (isGroup && isBotAdmin && !isAdmin) {
                    await sock.groupParticipantsUpdate(m.chat, [m.sender], "remove")
                }
            }
            db.antispam[m.sender] = now
        }

        // Antivirtex - deteksi pesan terlalu panjang
        if (budy.length > 5000) {
            m.reply("Terdeteksi pesan mencurigakan (virtex).")
            if (isGroup && isBotAdmin && !isAdmin) {
                await sock.groupParticipantsUpdate(m.chat, [m.sender], "remove")
            }
        }
